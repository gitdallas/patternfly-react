...
export const collapsible: IFormatter = (
  ...
  const {
    extraParams: { onCollapse, rowLabeledBy = 'simple-node', expandId = 'expand-toggle', allRowsExpanded }
  } = column;
  const extraData = {
    rowIndex,
    columnIndex,
  ...
  function onToggle(event: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
    ...
    onCollapse && onCollapse(event, rowIndex, !rowData?.isOpen, rowData, extraData);
  }
